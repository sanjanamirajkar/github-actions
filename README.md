...
It includes Docker build & push, security scans (DevSecOps), and deployment using a self-hosted runner.  
The application is a simple Flask app containerized and deployed using Docker Compose.  
This repository showcases end-to-end automation from code commit to deployment.
...
|---|---------|-----------------|-------------|
| 1 | [CICD](.github/workflows/cicd.yml) | `workflow_dispatch` inputs, `type: choice`, `needs` (job dependencies), `if` conditionals | Simulates a CI/CD pipeline (code → build → test → deploy) with manual environment selection |
| 2 | [Portfolio Deploy](.github/workflows/portfolio-deploy.yml) | `permissions`, pre-built actions (`checkout`, `configure-pages`, `deploy-pages`), environments, deployment URLs | Deploys a static site to GitHub Pages using official actions |
| 3 | [Python Lint](.github/workflows/python-matrix.yml) | `strategy.matrix`, `fail-fast: false`, `actions/setup-python`, dependency install | Lints `app.py` with flake8 across Python 3.9–3.13 in parallel |
| 4 | [Docker Build & Push](.github/workflows/docker-build-push.yml) | `workflow_call` (reusable workflow), `secrets`, `vars`, `docker/login-action`, `docker/build-push-action`, multi-tag images | Builds a Docker image and pushes to Docker Hub with branch, latest, and SHA tags |
| 5 | [Deploy App](.github/workflows/deploy-app.yml) | `workflow_run` (chained workflows), `self-hosted` runners, `env` context, Docker Compose | Auto-deploys the Flask app on a self-hosted server after Docker image is pushed |
...
│   │
│   │  # Part 1 — Fundamentals
│   ├── cicd.yml                # 1. Manual CI/CD — inputs, needs, if
│   ├── portfolio-deploy.yml    # 2. GitHub Pages — permissions, environments
│   ├── python-matrix.yml       # 3. Matrix — parallel linting
│   ├── docker-build-push.yml   # 4. Docker CI — build, tag, push (reusable)
│   ├── deploy-app.yml          # 5. Docker CD — workflow_run, self-hosted
│   │
│   │  # Part 2 — DevSecOps Pipeline
│   ├── devsecops-pipeline.yml  # 6. Orchestrator — chains all scans → build → deploy
│   ├── code-quality.yml        # 7. flake8 + bandit (lint + SAST)
│   ├── secrets-scan.yml        # 8. gitleaks (secrets in code)
│   ├── dependency-scan.yml     # 9. pip-audit (package CVEs)
│   ├── docker-lint.yml         # 10. hadolint (Dockerfile checks)
│   ├── image-scan.yml          # 11. Trivy (container image CVEs)
│   ├── deploy-to-server.yml   # 12. SSH + SCP deploy to EC2
│   └── tests.yml              # 13. pytest (route tests)
│
